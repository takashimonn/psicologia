...
const LoginScreen = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch('http://172.20.106.61:3000/login', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ usuario, contrasena, rol }), // Enviamos también el rol
      });
      ...
      if (data.exists && data.user) {
        ...
        const { rol, id_psicologo, id_paciente } = data.user; // Accede a los datos anidados
